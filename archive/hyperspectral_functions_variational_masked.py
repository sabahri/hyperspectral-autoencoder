...
def forward_pass(W,b,data_array):
	...
	# Bottleneck layer == Sampling Layer
	# Mean is just the layer, without activation
	mean = layer2 @ W[2] + b[2]
	std = softplus(layer2 @ weights3 + bias3) + 1e-6

	# epsilon for bottleneck reparamitrazation
	eps_bottle = np.random.normal(size=(,))

	z = mean + std*eps_bottle

	# Decoder
	#layer4 = relu(layer3 @ W[3] + b[3])
	layer4 = relu(z @ W[3] + b[3])
