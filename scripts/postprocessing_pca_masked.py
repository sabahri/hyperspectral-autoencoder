...
##### PCA projection (ceiling check: is class structure recoverable via a
##### plain linear projection, without any autoencoder in the loop at all?)
# Same Kneedle-derived bottleneck width as the trained networks, for a fair
# dimensionality comparison against the AE/VAE bottleneck clustering results.
...
# Top bott_dim eigenvectors (eigh returns ascending order, so take the last bott_dim)
top_eig_vec = eig_vec[:, -bott_dim:]
bottleneck = data_z_reshaped @ top_eig_vec
# Reconstruction via projection back up, for a per-pixel-loss comparison
# against the AE/VAE reconstruction error figures.
output = bottleneck @ top_eig_vec.T
...
plt.tight_layout()
plt.title('UMAP embedding of Bottleneck features (PCA, no autoencoder)')
plt.savefig('images/umap_bottleneck_pca_masked.png')
...
plt.tight_layout()
plt.savefig('images/perpixel_loss_pca_masked.png')
...
plt.savefig('images/kmc_gmm_ground_truth_pca_masked.png')
...
plt.suptitle('Per-dimension bottleneck activation histograms (PCA, no autoencoder)')
plt.tight_layout()

plt.savefig('images/pca_bottleneck_histograms_masked.png')
plt.show()
